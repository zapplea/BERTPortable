  ...
  def __init__(self,
    ...
    Raises:
      ValueError: The config is invalid or one of the input tensor shapes
        is invalid.
    """
    is_training = config['model']['is_training']
    use_one_hot_embeddings = False

    config = copy.deepcopy(config)
    if not is_training:
      config['model']['hidden_dropout_prob'] = 0.0
      config['model']['attention_probs_dropout_prob'] = 0.0
    ...
    with tf.variable_scope(scope, default_name="bert",reuse=tf.AUTO_REUSE):
      with tf.variable_scope("embeddings",reuse=tf.AUTO_REUSE):
        ...
        (self.embedding_output, self.embedding_table) = embedding_lookup(
            ...
            embedding_size=config['model']['hidden_size'],
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            token_type_vocab_size=config['model']['type_vocab_size'],
      ...
      with tf.variable_scope("encoder",reuse=tf.AUTO_REUSE):
        ...
        self.all_encoder_layers = transformer_model(
            ...
            intermediate_act_fn=get_activation(config['model']['hidden_act']),
...
def input_fn(config):
    input_ids = tf.placeholder(shape=(None,config['model']['max_seq_length']),dtype='int64')
    input_mask = tf.placeholder(shape=(None,config['model']['max_seq_length']),dtype='int64')
    segment_ids = tf.placeholder(shape=(None,config['model']['max_seq_length']),dtype='int64')
    masked_lm_positions = tf.placeholder(shape=(None,config['model']['max_predictions_per_seq']),dtype='int64')
    masked_lm_ids = tf.placeholder(shape=(None,config['model']['max_predictions_per_seq']),dtype='int64')
    masked_lm_weights = tf.placeholder(shape=(None,config['model']['max_predictions_per_seq']),dtype='int64')
    next_sentence_labels = tf.placeholder(shape=(None,1),dtype='int64')
